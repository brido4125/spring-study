    ...
    * */
    ...
    public String addItem(@ModelAttribute Item item, RedirectAttributes redirectAttributes,Model model) {
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            errors.put("price", "가격은 1,000원 ~ 1,000,000 사이의 값이어야 합니다.");
        }

        if (item.getQuantity() == null || item.getQuantity() >= 9999 || item.getQuantity() < 0) {
            errors.put("quantity", "수량은 0보다 크고 9999개 보다는 커야 합니다.");
        }
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10000) {
                errors.put("globalError", "가격 * 주문 수량은 10,000이상 이어야합니다. 현재 값 = " + resultPrice);
            }
        }

        //검증에 실패하면 다시 입력 Form으로 이동
        if (!errors.isEmpty()) { //map에 요소가 있으면,즉 에러가 있으면
