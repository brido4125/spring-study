...
public class ApplicationContextExtendsFindTest {
    ...
    @Test
    @DisplayName("부모 타입 조회시, 자식이 둘 이상 있으면 중복오류 발생")
    void findBeanByParentTypeDuplicatedFailureTest() {
        assertThrows(NoUniqueBeanDefinitionException.class, () -> ac.getBean(DiscountPolicy.class));
    }

    @Test
    @DisplayName("부모 타입 조회시, 자식이 둘 이상 있으면 중복오류 발생 시 이름 지정")
    void findBeanByParentTypeDuplicatedTest() {
        DiscountPolicy rateDiscountPolicy = ac.getBean("rateDiscountPolicy", DiscountPolicy.class);
        assertThat(rateDiscountPolicy).isInstanceOf(RateDiscountPolicy.class);
    }

    @Test
    @DisplayName("Specific Type Bean Find")
    void findBeanBySpecificTypeTest() {
